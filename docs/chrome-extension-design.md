...
- 右键菜单仍负责网页/选区/媒体捕获，并使用页面内可编辑预览浮层处理来源上下文。“保存媒体文件”和正文图片捕获都由浏览器扩展先下载源媒体并上传到后端附件接口；只有全部媒体下载并上传成功后，才会把正文中的原媒体 URL 替换为后端响应中的真实 `attachments/...` 路径并生成 prompt。由于后端会处理同名冲突并返回带 `-2`、`-3` 等后缀的真实落盘文件名，前端不得自行拼接或猜测附件路径。上传不完整时，捕获流程中止，不发送 chat。对图文混排内容，捕获服务会优先维护按 DOM 顺序生成的 `contentBlocks`，上传后的本地附件路径在该有序结构中替换，保证正文里的图片引用尽量停留在原网页相邻段落位置；媒体列表只作为附件核对清单，不作为正文排版依据。
- `extract-page` 页面正文抽取优先走站点适配器，再回退到通用正文抽取。通用抽取会在 `article` / `main` / `[role="main"]` 等正文容器内按 DOM 顺序提取标题、段落、列表、代码块、图片和图注，生成有序内容块，尽量保持普通博客和文档页的图文位置。当前已对 `x.com` / `twitter.com` 的 status/article 页面做专用适配：定位当前 URL 对应的主帖或长文 `article[data-testid="tweet"]` 及其所在 `cellInnerDiv`，只抽正文和正文图片，不包含评论；X 文本抽取不能只依赖 `tweetText`，还会把 `h1-h6`、段落、引用、代码块、`lang/dir` 文本容器和叶子 `div` 作为高价值正文候选，并过滤按钮、导航、计数、时间戳、用户名等低价值 UI 文案；正文和媒体会按页面 DOM 顺序输出为有序内容块，避免把所有图片统一追加到文末；图片会从 `tweetPhoto`、`/photo/`、`/media/` 链接、`pbs.twimg.com/media/` 节点以及 X 常用的 `background-image` 兜底图中去重收集，并把 X 的 `name=small` 等尺寸参数规范为 `name=large`。
